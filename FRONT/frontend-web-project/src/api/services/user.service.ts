...
export const getClient = (id: number): UseApiCall<Client> => {
    ...
    return {
        call: axios.get<Client>(`${BASE_URL}/${id}`, { signal: controller.signal }),
        controller
    };
}
...
export const getAllClients = (): UseApiCall<Client> => { // Puede estas muy mal
    const controller = loadAbort();
    return {
        call: axios.get<Client>(`${BASE_URL}`, { signal: controller.signal }),
        controller
    };
}
...
export const createClient = (client: Client): UseApiCall<Client> => {
    const controller = loadAbort();
    return {
        call: axios.post<Client>(`${BASE_URL}`, client, { signal: controller.signal }),
        controller
    };
}
...
export const updateClient = (id: number, client: Client): UseApiCall<Client> => {
    const controller = loadAbort();
    return {
        call: axios.put<Client>(`${BASE_URL}/${id}`, client, { signal: controller.signal }),
        controller
    };
}
...
export const deleteClient = (id: number): UseApiCall<void> => {
    const controller = loadAbort();
    return {
        call: axios.delete<void>(`${BASE_URL}/deleteClient/${id}`, { signal: controller.signal }),
        controller
    };
}